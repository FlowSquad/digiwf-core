...
public class SpringCloudStreamAdapter implements SendMessagePort, StartProcessPort, CorrelateMessagePort {
    ...
    @Override
    public boolean sendCorrelateMessage(final io.muenchendigital.digiwf.message.process.impl.model.Message<CorrelateMessageDto> message, final String destination) {
        return this.sendMessageToDestination(message.getPayload(), message.getHeaders(), destination);
    }

    @Override
    public boolean startProcess(final io.muenchendigital.digiwf.message.process.impl.model.Message<StartProcessDto> message, final String destination) {
        return this.sendMessageToDestination(message.getPayload(), message.getHeaders(), destination);
    }
